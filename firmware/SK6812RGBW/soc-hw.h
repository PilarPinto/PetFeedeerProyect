...
//Asigna la dirección del esclavo al cual se quiere leer o escribir
void i2c_addr(uint32_t addr);

//Asigna la operación que se quiere realizar sobre el esclavo (leer o escribir)
void i2c_rw(uint32_t rw);

//Asigna un valor a escribir, que puede se la dirección del puntero o datos
void i2c_dataWr(uint32_t data_wr);


//Obtiene el dato leido
uint32_t i2c_dataRd(void);


//Convierte de BCD a bin
...
//asigna el valor del registro enable
void i2c_enable(uint32_t ena);

//Obtiene el valor de busy
